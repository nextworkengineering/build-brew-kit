...
- Accent: the default is none (fully tonal), so the photos are the hero. If one is wanted: pumpkin, cornflower, plum, canary, sand, or emerald. Used as a small highlight on type (e.g. one word of a D headline); `none` = fully tonal. G stays photo-with-scrim, so ground mostly moves the block / text archetypes (D, build-master).
...
| `G-moment.html` | **Carousel moment.** Photo + one quiet centered narrative line (no giant time or hero word). Order them to tell the night. | `PHOTO META LINE` |
...
2. **A few moments** — G (moment), one beat each (doors → building → demos). A landscape moment? Use D (block + photo) instead of G, so the wide shot isn't force-cropped.
