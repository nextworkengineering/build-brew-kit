...
- Accent: the default is none (fully tonal), so the photos are the hero. If one is wanted: pumpkin, cornflower, plum, canary, sand, or emerald. In A it colours the big serif word; `none` = fully tonal. A sits on a paper ground (contain, text below the photo); G stays photo-with-scrim, so ground mostly moves the block / text archetypes.
